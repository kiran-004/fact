...
export default function Navbar({ onToggleSidebar, dark, setDark, user, onLogout }) {
  ...
  return (
    <header className="glass navbar-glass" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderRadius: 0, borderLeft: 'none', borderRight: 'none', borderTop: 'none', position: 'sticky', top: 0, zIndex: 30 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div className="glass" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px 6px 6px', borderRadius: 999 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600 }}>{user || 'Operator'}</span>
          <button onClick={onLogout} style={{ display: 'flex', alignItems: 'center', padding: 4, marginLeft: 4, borderRadius: '50%', color: 'var(--text-2)', transition: 'color 0.2s' }} className="btn-ghost" title="Sign out"><LogOut size={16} /></button>
        </div>
      </div>
    </header>
  );
